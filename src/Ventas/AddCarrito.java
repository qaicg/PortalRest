...
public class AddCarrito extends TestBase{
	...
	@Parameters({"productos","totalEsperado","opcionesMenu"})
	public void addCart(String productos, String totalEsperado, @Optional ("") String opcionesMenu) {
		     ...
		     if (familias.size()>0) {
	    	  ...
	    	  while (doScroll) {
		   		 ...
		   		 for(int i=1;i<= elements.size();i++) { 
			   		 ...
			   		 if(contieneNombre(arrayNombres,currentItem.getNombre()) 
			   				 && !productosAddeds.contains(currentItem) 
			   				 && (i<elements.size() || currentItem.getNombre().contains("MENÚ"))) { //SOLO AÑADIMOS UNA VEZ EL PRODUCTO Y SI NO ESTA EN SU ULTIMA POSICION, HACEMOS EXCEPCION CON MENÚ.
		   	   	     ...
		   	   	     if(vueltas>20 || productosEncontrados==arrayNombres.length) {
		   	   	    	 doScroll=false;
		   	   	    	 if(vueltas>20) {
		   	   	    		 log("No he encontrado todos los productos " + Arrays.toString(arrayNombres));
		   	   	    		 Assert.assertTrue(false);
		   	   	    	 }
	...
	private List<Integer> stringArrayToInteger(String values){
